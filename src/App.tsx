...
export default function App() {
  ...
  useEffect(() => {
    if (!gameEnded || scoreSaved || !currentUser) return;

    const result = score > 0 ? 'win' : score === 0 ? 'draw' : 'lose';
    addDoc(collection(db, 'scores'), {
      userId: currentUser.uid,
      displayName: currentUser.displayName ?? currentUser.email ?? '未知玩家',
      score,
      result,
      createdAt: serverTimestamp(),
    });
    setScoreSaved(true);
  }, [gameEnded, scoreSaved, currentUser, score]);
        ...
        {gameEnded && (
          ...
          >
            <div className="mb-4 p-6 bg-amber-200/80 backdrop-blur-sm rounded-xl shadow-lg border-2 border-amber-400">
              ...
              <p className={`text-4xl font-bold mb-2 ${score > 0 ? 'text-green-600' : score === 0 ? 'text-amber-600' : 'text-red-600'}`}>
